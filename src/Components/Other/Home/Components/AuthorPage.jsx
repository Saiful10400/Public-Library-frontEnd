      ...
      .then((res) =>{
        ...
        if(person){
            ...
            .then(res=>{
                ...
                const isFollowing=author?.followers?.find(item=>item===userData._id)
...
const followBehaviourHandle=(authorId)=>{
    if(user){
        if(!isFollowing){
          axiosPublic.post("/follow_a_author",{userId:user._id,authorId})
        .then(res=>{
            console.log(res.data)
            setReload(!reload)
        })
        }else{
          axiosPublic.post("/unfollow_a_author",{userId:user._id,authorId})
          .then(res=>{
              console.log(res.data)
              setReload(!reload)
          })
    ...
    <div>
      ...
      <div className="flex gap-2 py-4">
        <div className="border-2 w-[20%] flex flex-col justify-center items-center">
          ...
          <h1 className={author?.followers?.length?"font-bold mt-4 opacity-100 transition-all duration-300":"opacity-0"}>
            {author?.followers?.length} following
          </h1>

          <button onClick={()=>followBehaviourHandle(author?._id)} className=" mt-1 flex justify-center items-center font-bold bg-primary text-white text-md px-3 py-2">
            <FaPlus />
            <span>Follow</span>
          </button>
        </div>
